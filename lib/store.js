...
function bookingFromRow(row) { return { id: row.id, co: row.company, show: row.show || "", note: row.note || "", dates: row.dates || [] }; }
...
async function runSync(blob, isAdmin) {
  ...
  try {
    ...
    for (const m of blob.ojt?.months || []) {
      const h = hashOf(m);
      monthHash[m.m] = h;
      if (prevMonthHash[m.m] !== h) await post("/api/ojt-months", m);
    }
...
export const store = {
  ...
  async load() {
    ...
    try {
      ...
      const [profileRes, showsRes, flagsRes, entriesRes, ojtRes, bookingsRes, classesRes, ratesRes, pinsRes] = await Promise.all([
