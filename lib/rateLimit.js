...
export async function checkRateLimit(supabase, key, max, windowSeconds) {
  ...
  if (error) {
    // Fails closed above, but log so a broken RPC (missing function/table,
    // bad grants) doesn't look identical to a real rate-limit hit.
    console.error("check_rate_limit RPC failed, blocking request:", error.message);
    return false;
  }
  return data === true;
}
