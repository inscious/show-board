...
function readSyncState(): SyncState {
    try {
        return JSON.parse(lsGet(SYNC_KEY) || "") || {};
    } catch {
        return {};
    }
}
function writeSyncState(s: SyncState): void {
    lsSet(SYNC_KEY, JSON.stringify(s));
}
...
function showFromRow(row: ShowRow, flag?: ShowFlagRow): Show {
    return {
        ...
        co: row.gc || "",
        region: row.region || "",
        src: row.source || "union",
        sheetMonth: row.sheet_month || "",
        status: flag ? flag.status : null,
        note: flag ? flag.note || "" : "",
    };
}
function entryFromRow(row: WorkEntryRow): Entry {
    const e: Entry = {
        ...
        cat: row.category,
        note: row.note || "",
        hrs: Number(row.hours),
    };
    if (row.in_min != null && row.out_min != null) {
        e.in = row.in_min;
        e.out = row.out_min;
...
function classFromRow(row: ClassRow): Klass {
    return {
        id: row.id,
        name: row.name,
        start: row.start_min,
...
async function runSync(blob: SyncBlob, isAdmin: boolean): Promise<void> {
    ...
    // rates
    try {
        const rateCos = Object.keys(blob.rates || {}).filter(
            (co) => blob.rates[co],
        );
    ...
    try {
        ...
        for (const name of pins)
            if (!(prev.pins || []).includes(name))
                await post("/api/pins", { name, pinned: true });
        for (const name of removedIds(prev.pins, pins))
            await post("/api/pins", { name, pinned: false });
        next.pins = pins;
        persist();
    } catch (err) {
        failures.push({ cat: "pins", err });
    }

    // custom companies — additive only, the app never removes one
    try {
        const customCos = blob.customCos || [];
        for (const name of customCos)
            if (!(prev.customCos || []).includes(name))
                await post("/api/custom-companies", { name });
        next.customCos = customCos;
        persist();
    } catch (err) {
        failures.push({ cat: "customCos", err });
    }
    ...
    try {
        ...
        for (const s of blob.shows) {
            ...
            flagHash[s.id] = fh;
            if (prevFlagHash[s.id] !== fh)
                await post("/api/show-flags", flagBody);
        }
        if (isAdmin) {
            for (const id of removedIds(
                Object.keys(prevShowHash),
                Object.keys(showHash),
            ))
                await del("/api/shows", { id });
        }
        next.shows = showHash;
        next.showFlags = flagHash;
        persist();
    } catch (err) {
        failures.push({ cat: "shows", err });
    }
    ...
    if (failures.length === 0) {
        ...
        const rateLimited = failures.some((f) =>
            / 429$/.test(String((f.err as Error)?.message)),
        );
        setSyncStatus({
            ok: false,
            message: rateLimited
...
function syncToRemote(blob: SyncBlob, isAdmin: boolean): Promise<void> {
    ...
    syncing = true;
    currentSync = runSync(blob, isAdmin).finally(() => {
        syncing = false;
    });
    return currentSync;
}
...
if (typeof window !== "undefined") {
    window.addEventListener("online", () => {
        if (lastBlob) syncToRemote(lastBlob, !!lastBlob.__isAdmin);
    });
}
...
export const store = {
    ...
    async load(): Promise<Blob | null> {
        ...
        try {
            cachedData = cached !== null ? JSON.parse(cached) : null;
        } catch {}
        ...
        try {
            ...
            const [
                ...
                supabase
                    .from("profiles")
                    .select(
                        "is_admin, has_password, custom_companies, name, member_id, ssn_last4, local, rsi_credits, joined_on, do_not_hire_at, do_not_hire_reason, approved_at, welcomed_at, avatar_url, graduated_at",
                    )
                    .eq("id", user.id)
                    .single(),
            ...
            if (cachedData) {
                store.isAdmin = !!cachedData.isAdmin;
                store.email = cachedData.email ?? null;
                store.hasPassword = !!cachedData.hasPassword;
            }
            return cachedData;
        }
    },
    ...
    async signOut(): Promise<void> {
        ...
        if (lastBlob && navigator.onLine) {
            const flush = currentSync.then(() =>
                syncToRemote(lastBlob!, !!lastBlob!.__isAdmin),
            );
            await Promise.race([
                flush,
    ...
    async setPassword(
        ...
        try {
            ...
            if (!res.ok)
                return {
                    ok: false,
                    error: body.error || "Couldn't set password",
                };
    ...
    async exportJson(): Promise<string> {
        return lsGet(STORE_KEY) || "{}";
    },
    async importJson(json: string): Promise<boolean> {
        try {
            JSON.parse(json);
            return lsSet(STORE_KEY, json);
        } catch {
            return false;
        }
    },
